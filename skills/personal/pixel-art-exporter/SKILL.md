...
Before exporting, read the matching section of `examples.md` for a worked export: **Single Image Exports**, **Animated GIF Exports**, **Spritesheet Exports**, or **Game Engine Integration**. Read `export-formats.md` when you need exact PNG/GIF options, spritesheet layout geometry, a JSON metadata schema, or scaling rules.
