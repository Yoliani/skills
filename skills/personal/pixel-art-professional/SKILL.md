...
Before applying a technique, read the matching section of `examples.md` for a worked before/after: **Dithering Applications**, **Palette Optimization**, **Shading Techniques**, **Antialiasing**, or **Complete Refinement Workflows**. Read `reference.md` when you need dithering algorithm details, color-ramp theory, a classic palette, light-source shading rules, or the common-mistakes catalogue. `dithering-patterns.md` holds the full pattern library.

**Tool-call syntax in the sibling files is stale** — verify parameter names against the live MCP schema before copying any call. Use the prose and structure, not the literal arguments.
