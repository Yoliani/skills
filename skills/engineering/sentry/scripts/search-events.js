...
import { API_BASE, getDefault, requireSentry, getAuthToken, fetchJson, formatTimestamp, resolveProjectId } from "../lib/api.js";

const HELP = `Usage: search-events.js [options]

Search for events (transactions, errors) in Sentry Discover.

Sentry only - GlitchTip has no Discover API. On GlitchTip use list-issues.js
and then fetch-issue.js --latest instead.
...
async function main() {
  ...
  requireSentry(
    ...
    "Use ./scripts/list-issues.js to find issues, then ./scripts/fetch-issue.js <id> --latest for the event."
  );
