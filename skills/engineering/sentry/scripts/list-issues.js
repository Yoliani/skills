...
List and search issues in Sentry or GlitchTip.
...
Query Syntax (GlitchTip supports the is:/level: subset):
...
async function main() {
  ...
  if (options.projects.length === 0) {
    const defaultProject = getDefault("project");
    if (defaultProject) options.projects.push(defaultProject);
  }
  ...
  if (options.period) {
    if (isGlitchTip) {
      // GlitchTip has no statsPeriod; it takes an explicit start/end window.
  ...
  if (options.sort) {
    // GlitchTip uses its own sort keys instead of Sentry's.
    const glitchTipSort = {
      date: "-last_seen",
      new: "-first_seen",
      freq: "-count",
    ...
    if (isGlitchTip) {
      const mapped = glitchTipSort[options.sort];
      if (!mapped) {
        console.error(`Error: GlitchTip does not support --sort ${options.sort}`);
        console.error(`Supported: ${Object.keys(glitchTipSort).join(", ")}`);
        process.exit(1);
      }
  ...
  const url = `${API_BASE}/organizations/${encodeURIComponent(options.org)}/issues/?${params.toString()}`;
