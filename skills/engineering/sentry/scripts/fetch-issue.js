...
import { API_BASE, ROOT_URL, isGlitchTip, getDefault, getAuthToken, fetchJson, formatTimestamp } from "../lib/api.js";
...
function parseIssueInput(input) {
  ...
  // Any other issue URL, e.g. a GlitchTip instance:
  // https://glitchtip.example.com/myorg/issues/42
  const genericUrlMatch = input.match(/^https?:\/\/[^/]+\/(?:organizations\/)?([^/]+)\/issues\/(\d+)/);
  if (genericUrlMatch) {
    return { org: genericUrlMatch[1], issueId: genericUrlMatch[2] };
  }
...
function formatIssue(issue) {
  ...
  const link = issue.permalink || (issue.project?.slug ? `${ROOT_URL}/${issue.project.slug}/issues/${issue.id}` : null);
  if (link) {
    lines.push(`**URL:** ${link}`);
  }
...
async function main() {
  ...
  if (args.length === 0 || args.includes("--help") || args.includes("-h")) {
    ...
    console.log("  --url <url>  Server root URL (default: ~/.sentryclirc [defaults] url)");
    console.log("  --backend <sentry|glitchtip>  Override backend detection");
    console.log("");
  ...
  const cliOrg = orgIndex !== -1 ? args[orgIndex + 1] : getDefault("org");
  ...
  try {
    ...
    if (parsed.shortId) {
      if (isGlitchTip) {
        console.error("Error: GlitchTip has no short-ID lookup endpoint.");
        console.error("Use the numeric issue ID or the issue URL instead, or find it with:");
        console.error("  ./scripts/list-issues.js --query <search terms>");
        process.exit(1);
      }
      ...
      const shortIdUrl = `${API_BASE}/organizations/${org}/shortids/${encodeURIComponent(parsed.shortId)}/`;
      ...
      const issueUrl = `${API_BASE}/issues/${parsed.issueId}/`;
    ...
    if (wantLatest) {
      const eventUrl = `${API_BASE}/issues/${issue.id}/events/latest/`;
