...
The reviewer is a different agent instance from the implementer even when both are `pi` — a fresh context is the point. Once an instance exists, reuse it for every later round; a returning reviewer remembers what it asked for.
...
**Done when** every question your reading raised is either answered by evidence you gathered or answered by the user, and the brief states the boundary explicitly. Unanswered questions in the brief are fog you failed to map — dispatching with them is what this step exists to prevent.
...
If a fix reveals the brief was wrong — same rule as step 2 — stop and re-map the fog with the user.
...
Leave the agents running — the user may want another round. Close nothing you did not create.
