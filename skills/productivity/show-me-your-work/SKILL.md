...
description: Keep a reviewable decision trail for long-running or unattended work — a TSV log with one row per decision (what, why, evidence, result).
...
- Prefer evidence produced by committed scripts over hand-made one-offs, so a reviewer can re-run it instead of trusting a one-time claim.

## Audit the log against the transcript

At the end of the run, before handing back, check the log told the truth. Read this run's transcript wherever your harness stores it (in Claude Code, the current session's `.jsonl` under `~/.claude/projects/<project-dir>/`); if the harness exposes no transcript, audit against the evidence pointers alone. Stay inside the current session's file; other sessions are unrelated private chats. Walk the log against what actually happened:
...
Before handing back, you must spawn a subagent on a different model from the one that did the work (in Claude Code, pass a `model` override to the Agent tool; in other harnesses, use their subagent mechanism's model selection). If your harness can't select a different model, still spawn a fresh-context subagent and note in the Attention section that the review was same-model. Self-review is not a substitute; the point is fresh eyes you cannot bring yourself. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
