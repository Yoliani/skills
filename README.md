...
- **[posthog](./skills/engineering/posthog/SKILL.md)**: Read product analytics and error tracking from PostHog (cloud or self-hosted): run HogQL, triage error tracking issues with stack traces, read feature flags and saved insights, find session recordings, and annotate deploys.
- **[sonarqube](./skills/engineering/sonarqube/SKILL.md)**: Read code quality from SonarQube Server or Cloud: quality gate status and the conditions behind it, coverage, duplication and technical debt measures, their history across analyses, and the issues underneath.
...
The `native-web-search` and `web-browser` skills are adapted from [mitsuhiko/agent-stuff](https://github.com/mitsuhiko/agent-stuff).
